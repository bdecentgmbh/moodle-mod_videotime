 ...
 * needed to restore one videotimeplugin_vimeo subplugin.
 */
class restore_videotimeplugin_vimeo_subplugin extends restore_subplugin {
    /**
    ...
    protected function define_videotime_subplugin_structure() {
        ...
        $elepath = $this->get_pathfor('/vimeo_settings');
    ...
    /**
     * Processes the videotimeplugin_vimeo element, if it is in the file.
     * @param array $data the data read from the XML file.
     */
    public function process_videotimeplugin_vimeo($data) {
        global $DB;

        $data = (array) $data + (array) get_config('videotimeplugin_vimeo');
        $data = (object)$data;
        $data->videotime = $this->get_new_parentid('videotime');
        $DB->insert_record('videotimeplugin_vimeo', $data);
    }
}
