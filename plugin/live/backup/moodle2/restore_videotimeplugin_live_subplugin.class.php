 ...
 * needed to restore one videotimeplugin_live subplugin.
 */
class restore_videotimeplugin_live_subplugin extends restore_subplugin {
    /**
    ...
    /**
     * Processes the videotimeplugin_live element, if it is in the file.
     * @param array $data the data read from the XML file.
     */
    public function process_videotimeplugin_live($data) {
        global $DB;

        $data = (array) $data + (array) get_config('videotimeplugin_live');
        $data = (object)$data;
        $data->videotime = $this->get_new_parentid('videotime');
        $DB->insert_record('videotimeplugin_live', $data);
    }
    ...
    protected function after_execute(): void {
        ...
        $this->add_related_files('videotimeplugin_live', 'mediafile', null);
    }
}
