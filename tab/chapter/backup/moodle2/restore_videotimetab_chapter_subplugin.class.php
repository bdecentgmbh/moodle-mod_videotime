 ...
 * needed to restore one videotimetab_chapter subplugin.
 */
class restore_videotimetab_chapter_subplugin extends restore_subplugin {
    ...
    protected function define_videotime_subplugin_structure() {
        ...
        $elepath = $this->get_pathfor('/videotimetab_chapter');
    ...
    /**
     * Processes the videotimetab_chapter element, if it is in the file.
     * @param array $data the data read from the XML file.
     */
    public function process_videotimetab_chapter($data) {
        global $DB;

        $data = (object)$data;
        $oldvideotime = $data->videotime;
        $data->videotime = $this->get_new_parentid('videotime');
        $DB->insert_record('videotimetab_chapter', $data);
    }
}
