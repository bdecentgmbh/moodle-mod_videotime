 ...
 * needed to restore one videotimetab_chat subplugin.
 */
class restore_videotimetab_chat_subplugin extends restore_subplugin {
    /**
    ...
    protected function define_videotime_subplugin_structure() {
        ...
        $elepath = $this->get_pathfor('/videotimetab_chat');
    ...
    /**
     * Processes the videotimetab_chat element, if it is in the file.
     * @param array $data the data read from the XML file.
     */
    public function process_videotimetab_chat($data) {
        global $DB;

        $data = (object)$data;
        $oldvideotime = $data->videotime;
        $data->videotime = $this->get_new_parentid('videotime');
        $DB->insert_record('videotimetab_chat', $data);
    }
}
