...
class restore_videotimetab_block_subplugin extends restore_subplugin {
    ...
    protected function define_videotime_subplugin_structure() {
        ...
        $elepath = $this->get_pathfor('/videotimetab_block');
    ...
    /**
     * Processes the videotimetab_block element, if it is in the file.
     * @param array $data the data read from the XML file.
     */
    public function process_videotimetab_block($data) {
        global $DB;

        $data = (object)$data;
        $oldvideotime = $data->videotime;
        $data->videotime = $this->get_new_parentid('videotime');
        $DB->insert_record('videotimetab_block', $data);
    }
}
